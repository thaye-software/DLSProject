...
RUN --mount=type=secret,id=database_url_prod \
    ...
    sh -c 'if [ -f /run/secrets/database_url_prod ]; 
    then DATABASE_URL_PROD="$(cat /run/secrets/database_url_prod)"; elif [ -f /run/secrets/database_url_dev ]; 
    then DATABASE_URL_DEV="$(cat /run/secrets/database_url_dev)"; fi; if [ -f /run/secrets/app_env ]; 
    then APP_ENV="$(cat /run/secrets/app_env)"; echo "Build app_env=$APP_ENV"; fi; if [ -f /run/secrets/supabase_url ]; 
    then NEXT_PUBLIC_SUPABASE_URL="$(cat /run/secrets/supabase_url)"; fi; if [ -f /run/secrets/supabase_key ]; 
    then NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY="$(cat /run/secrets/supabase_key)"; fi; if [ -f /run/secrets/stripe_secret_key ]; 
    then STRIPE_SECRET_KEY="$(cat /run/secrets/stripe_secret_key)"; fi; if [ -f /run/secrets/stripe_public_key ]; 
    then NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY="$(cat /run/secrets/stripe_public_key)"; fi; if [ -f /run/secrets/resend_key ]; 
    then RESEND_API_KEY="$(cat /run/secrets/resend_key)"; fi; if [ -f /run/secrets/resend_origin_email_key ]; 
    then RESEND_ORIGIN_EMAIL="$(cat /run/secrets/resend_origin_email_key)"; fi;
    then POSTHOG_API_KEY="$(cat /run/secrets/posthog_key)"; fi; if [ -f /run/secrets/posthog_host ]; 
    then POSTHOG_HOST="$(cat /run/secrets/posthog_host)"; fi;
    export DATABASE_URL_PROD DATABASE_URL_DEV APP_ENV NEXT_PUBLIC_SUPABASE_URL NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY STRIPE_SECRET_KEY NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY RESEND_API_KEY RESEND_ORIGIN_EMAIL; npm run build'
