                    ...
                    <div className="space-y-8">
                        ...
                        <div className="border-b border-[#D3C6A3] pb-6">
                            ...
                            <p className="text-[#244B5A] text-sm">
                                Reference: {product.watch.reference}
                            </p>
                        </div>
                        ...
                        <div className="bg-white border border-[#D3C6A3] rounded-lg p-6 shadow-sm">
                            ...
                            <div className="text-[#5E561C] text-xs mt-2">
                                Including {product.watch.vat}% VAT
                            </div>
                        </div>

                        {/* Description */}
                        <div>
                            <h2 className="text-xl font-semibold text-[#1A1A1A] mb-3">Description</h2>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <SpecItem icon={<Gauge size={18} />} label="Size" value={product.watch.size || "Not specified"} />
                                <SpecItem icon={<Clock size={18} />} label="Movement" value={product.watch.movement || "Not specified"} />
                                <SpecItem icon={<Calendar size={18} />} label="Year" value={product.watch.year.toString() || "Not specified"} />
                        ...
                        <div className="flex items-center gap-2 text-sm">
                            {product.stock > 0 ? (
                                <>
                                    ...
                                    <span className="text-[#2D4330] font-medium">In Stock ({product.stock} available)</span>
                                </>
                            ) : (
                                <>
                                    <div className="w-2 h-2 bg-[#773D0E] rounded-full"></div>
                                    <span className="text-[#773D0E] font-medium">Out of Stock</span>
                                </>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-6 border-t border-[#D3C6A3]">
                            ...
                            <TrustBadge icon={<Package size={24} />} text="Insured Shipping" />
                            <TrustBadge icon={<FileText size={24} />} text="2 Year Warranty" />
                        </div>
                    </div>
                </div>
            </div>
...
function IncludedItem({ included, text }: { included: boolean; text: string }) {
    return (
        <div className="flex items-center gap-3">
            ...
            <span className={included ? "text-[#244B5A]" : "text-[#5E561C] line-through"}>
                {text}
            </span>
        </div>
    );
